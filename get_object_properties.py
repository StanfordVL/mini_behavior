import json
from mini_behavior.bddl import ALL_STATES
from mini_behavior.bddl import OBJECTS
...
property_objs['all'] = OBJECTS

obj_properties = {}

for name, obj in property_objs.items():
    props = []
    for prop in obj.keys():
        # filter out states that we don't implement
        if prop in ALL_STATES:
            props.append(prop)
    obj_properties[name] = props

for obj, props in obj_properties.items():
    print(f'{obj}: {props}')
